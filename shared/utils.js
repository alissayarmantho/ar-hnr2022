...
class Reticle extends THREE.Object3D {
  ...
  loadObjectList(objectList) {
    var newObjects = [];
    for (let i = 0; i < objectList.length; i++) {
      this.loader.load(objectList[i].link, (gltf) => {
        const object = gltf.scene.children.find(
          (c) => c.name === objectList[i].name
        );
        newObjects.push(gltf.scene);
      });
    }
...
window.gltfLoader.load(
  "https://alissayarmantho.github.io/ar-hnr2022/assets/sunflower.gltf",
  function (gltf) {
